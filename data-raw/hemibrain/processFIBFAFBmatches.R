...
lhn_matching <- function(lh.meta,
  ...
  initials = hemibrainr:::must_be("Choose a user : ", answers = unique(gs$User))
  lh.meta = subset(lh.meta, lh.meta$bodyid %in% gs$bodyid)
  rgl::bg3d("white")
  for(n in lh.meta$bodyid){
    ...
    fafb = catmaid::read.neurons.catmaid(names(r)[1:10])
    sel = c("go","for","it")
    k = 1
    j = 10
    ...
    while(length(sel)>1){
      ...
      prog = hemibrainr:::hemibrain_choice(sprintf("You selected %s neurons. Are you happy with that? ",length(sel)))
      if(!prog){
        sel = c("go","for","it")
