...
for (c in cts){
  ...
  if(skid%in%names(neurons)){
    ...
    neuron = elmr::fetchn_fafb(skid,mirror=FALSE,reference = JFRC2013)
    # Get meta info
...
devtools::use_data(emlhns,overwrite=TRUE,compress=FALSE)
devtools::use_data(emlhns.dps,overwrite=TRUE,compress=FALSE)
